...
<body>
				...
				<div class="row">
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow min-h200">
							...
							<div id="gangweileixingMain" class="graph" style="width: 900px;height:600px;"></div>
						</div>
					</div>
				</div>
			</div>
	...
	var gangweileixingValArr = [];
	var gangweileixingNameArr = [];
	var gangweileixingMapList = [];
	var valueArr = []
		var graphType = "pie";

	var gangweileixingVal = '';
			...
			series : [
				{
					...
					data:[          // 数据数组，name 为数据项名称，value 为数据项值
						{value:gangweileixingVal, name:'岗位类型'},
					]
				}
			]
		})
	...
	function gangweileixingDraw(){
		// 基于准备好的dom，初始化echarts实例
		var myChart = echarts.init(document.getElementById('gangweileixingMain'),'macarons');
			...
			series : [
				{
					...
					type: 'pie',    // 设置图表类型为饼图
					radius: '55%',  // 饼图的半径，外半径为可视区尺寸（容器高宽中较小一项）的 55% 长度。
					data: gangweileixingMapList   // 数据数组，name 为数据项名称，value 为数据项值
				}
			]
		})
	}
	function getDetails(){
				group('gangweileixing')
	}
	// 值 字段 数据绑定
	...
	function groupDataBind(list,colName){
				if(colName == "gangweileixing"){ 
						for(var i=0;i<list.length;i++){
							var map = {value: list[i].total,name: list[i].gangweileixing}
							gangweileixingMapList.push(map)
						}
					gangweileixingDraw()
				}
