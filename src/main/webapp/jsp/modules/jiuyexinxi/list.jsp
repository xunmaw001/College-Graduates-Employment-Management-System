...
<body>
						...
						<div class="widget-area-2 lochana-box-shadow">
							...
							<div class="table-responsive mb-3" id="tableDiv">
								<div class="col-sm-12">
									...
									<input type="text" id="qiyezhanghaoSearch" class="form-control form-control-sm" placeholder="请输入企业账号" aria-controls="tableId">
									<label>
										岗位类型
									</label>
									<select name="gangweileixing" id="gangweileixingSelect" class="form-control form-control-sm" aria-controls="tableId">
									</select>
                                    <label>
                                        是否通过
                                    </label>
								...
								<table id="tableId" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th onclick="sort('gangweileixing')">岗位类型<i id="gangweileixingIcon" class="fa fa-sort"></i></th>
	...
	<script>
		...
		function search(){
			...
			var gangweileixingIndex = document.getElementById("gangweileixingSelect").selectedIndex;
			var gangweileixingOptions = document.getElementById("gangweileixingSelect").options;
			var gangweileixingSelectedOption = gangweileixingOptions[gangweileixingIndex]
			if(gangweileixingSelectedOption.text != null && gangweileixingSelectedOption.text != ''){
				searchForm.gangweileixing = gangweileixingSelectedOption.text;
			}
        ...
        function getDataList() {
			http("jiuyexinxi/page","GET",{
				...
				gangweileixing : searchForm.gangweileixing,	
			},(res)=>{
				if(res.code == 0){
					clear();
		...
		function setDataRow(item,number){
			...
			var gangweileixingCell  = document.createElement('td');

						 gangweileixingCell.innerHTML = item.gangweileixing;

			row.appendChild(gangweileixingCell);
			...
			var sanfanghetongCell  = document.createElement('td');

						  var sanfanghetongFile = document.createElement('button');
			  sanfanghetongFile.setAttribute('class','btn-table')
			  sanfanghetongFile.innerHTML = '下载';
			  var sanfanghetongFileAttr = "download('" +baseUrl+ item.sanfanghetong + "')"
			  sanfanghetongFile.setAttribute('onclick',sanfanghetongFileAttr);
        	          sanfanghetongCell.appendChild(sanfanghetongFile);


			row.appendChild(sanfanghetongCell);
			...
			var crossBtn0 = document.createElement('button');
            		crossBtn0.innerHTML = "就业派遣"
			crossBtn0.setAttribute("type","button");
			crossBtn0.setAttribute("class","btn btn-primary btn-sm");
			crossBtn0.setAttribute("onclick","crossTable(" + JSON.stringify(item) + ",'jiuyepaiqian','是','','')");
			if(crossBtnControl('就业派遣') == true){
		...
		function sh(){
			...
			httpJson("jiuyexinxi/update","POST",data,(res)=>{
				if(res.code == 0){
					alert("提交成功");
					getDataList();
				}
